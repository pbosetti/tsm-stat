...
ita_f <- ita_f %>%
  ...
  mutate(cases_new_pt = cases_new/Popolazione*1000,
         deaths_new_pt = deaths_new/Popolazione*1000)
...
# Plotto decessi
ita_f_sel %>%
  ggplot(aes(x=date, y=deaths_new_pt, color=region)) + 
  geom_line() +
  labs(x = "Data", y="Decessi per mille abitanti", color="Regione") +
  scale_x_date(date_breaks = "1 month", labels=scales::label_date("%d %b")) +
  scale_y_continuous(labels = scales::label_number()) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
