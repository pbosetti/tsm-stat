...
* assume sempre un default di 30 bins, mentre `hist()` li calcola automaticamente con la formula di Sturges, $k=\left\lceil{\log_2(n)}\right\rceil + 1$, fornita da `nclass.Sturges()`;
...
ggplot(data=diamonds) + 
  stat_summary(mapping=aes(x=cut, y=price), 
               ...
               fun=median)
```
Infine, la distribuzione può essere analizzata mediante i diagrammi quantile-quantile. La libreria `qqplotr` mette a disposizione la funzione `geom_qq_band()` per aggiungere anche la banda di confidenza. Si noti che per default tale funzione calcola la banda assumendo una distribuzione a valore atteso uguale alla media del campione, mentre `geom_qq()` assume media nulla: è quindi necessario specificare anche l'opzione `dparams=list(mean=0)`:
...
ggplot(aes(sample=x)) + 
  stat_qq_band(dparams=list(mean=0)) +
  geom_qq() +
...
I boxplot sono grafici molto utili per comparare visivamente la distribuzione di due o più parametri. In questo caso vediamo anche l'aggiunta degli *intagli* (o *notch*), che servono a verificare rapidamente la significatività della differenza: se i *notch* non sono sovrapposti possiamo rigettare l'ipotesi nulla di uguaglianza delle medie:
...
df %>% ggplot(aes(x=treat, y=yield)) +
  geom_boxplot(fill="steelblue", notch=T)
```

### Istogrammi marginali

In caso di dati bivariati sono utili i cosiddetti *istogrammi marginali*, ovvero istogrammi sui lati di un grafico a dispersione corripondenti alla distribuzione dei punti proiettati sugli assi:
...
Creiamo un diagramma di Pareto a partire dai dati estesi, ossia raggruppando dati puntuali in classi. In particolare, usiamo le funzioni `dplyr::ntile()` per creare un certo numero (10) di classi equi-popolate, e usiamo invece `cut()` per definire classi basate su intervalli arbitrari.
...
nc <- 10
pop <- tibble(
  c = rbeta(100, 1, 500)*10000,
  i = seq_along(c)
) %>% 
  relocate(i, .before = c) %>%
  mutate(
    qclass = ntile(c, nc),
    class = cut(c, breaks=c(0, 25, 50, 60, Inf))) %>%
  arrange(qclass, c)
str(pop)
```

Ora visualizziamo i dati come grafico a dispersione, colorando i punti secondo la classe quantile e con forma secondo la classe arbitraria:

```{r}
pop %>% ggplot() +
  geom_point(aes(x = i, y=c, color = qclass, shape = class), 
             size = 3, alpha =3/4) + 
  scale_shape(name = "Classe arbitraria") +
  scale_color_viridis_b(option = "A", breaks = 1:nc, name = "Classe quantile") +
  labs(x = "Indice", y = "Valore")
```
Infine, raggruppiamo i dati per classe e calcoliamo la somma per classe; calcoliamo anche la somma cumulativa (ma in ordine inverso, dalla classe con somma più grande a discendere):

```{r}
pop %>% 
  group_by(qclass) %>%
  summarise(n = n(), sum = sum(c), min = min(c), max = max(c)) %>%
  mutate(ord = rev(seq_along(qclass))) %>%
  arrange(ord) %>%
  mutate(cum = cumsum(sum)) %>%
  ggplot(aes(x = ord, y = sum)) +
  geom_bar(aes(fill = sum), stat="identity") +
  geom_line(aes(y = cum)) + 
  geom_point(aes(y = cum)) +
  geom_label_repel(aes(y = cum, 
                       label = paste0(round(cum/sum(sum)*100), "%")),
                   nudge_x = -0.5, 
                   nudge_y = 250, 
                   size = 3) + 
  scale_x_continuous(breaks = 1:nc, labels = ~paste0(.x/nc*100, "%")) +
  scale_fill_viridis_b(name = "Parziali") +
  labs(x = "Frazione della popolazione", y = "Parziali")
```
Le prime 4 classi, che raccolgono il 40% della popolazione, contribuiscono per il 76% del valore complessivo.
...
Realizziamo un grafico separando visvamente le anomalie negative da quelle positive, e indicando in maniera differente i valori successivi al primo Gennaio 2000:
...
```
**NOTA**: Per formattare le etichette dell'asse delle ascisse come date è necessario usare la funzione `label_date()` della libreria `scales`. Tuttavia, purtroppo questa libreria è in conflitto con la libreria `readr` (sopra caricata), in quanto entrambe definiscono una funzione `col_factor()`. In questi casi, anziché caricare la libreria con il comando `library()` si può accedere alla singola funzione con la notazione `scales::label_date()`.
...
if (Sys.info()["sysname"] == "Windows") {
  invisible(Sys.setlocale("LC_TIME", "Italian"))
} else {
...
ctsm %>%
  ggplot(aes(x=new_cases, y=positive_rate*100, color=new_tests)) +
...
Per finire, vediamo come filtrare i dati dell'oggetto `xts` originario con `ksmooth()`. Quest'ultima funzione si applica *necessariamente* su una coppia `x` e `y`, quindi la trasformazione non può purtroppo essere effettuata al volo (come `rollmedian()`) ma è necessario creare un nuovo oggetto `xts` multivariato, passando per un `data.frame`.
...
last(ctsf, "3 months") %>%
  ...
  geom_line() +
  geom_hline(yintercept = 0) +
  geom_line(aes(x=Index, y=new_deaths * coef), color=deaths_color) +
  labs(x="Data", y="Nuovi casi/giorno", 
       title="Situazione COVID-19 in Italia",
       subtitle="Ultimi 3 mesi") + 
  scale_x_date(date_breaks = "7 days", labels=scales::label_date("%d %b'%y")) +
  scale_y_continuous(
    labels = scales::label_number_si(),
    sec.axis=sec_axis(~./coef, name="Nuovi decessi/giorno")
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.y.right = element_text(color=deaths_color))
```

Questo esempio illustra anche come plottare due serie su due assi $y$ diversi (destro e sinistro).
Ancora più interessante è plottare il grafico della mortalità \footnote{Si noti che in realtà questa è un'approssimazione dell'indice di mortalità, dato che è il raporto tra nuovi casi e nuovi decessi al giorno (inoltre filtrati!), e non la probabilità di sopravvivenza!}:
...
ctsf %>% 
  ...
  geom_area(fill=grey(0.8), alpha=0.5) +
  geom_line() + 
  geom_line(aes(x=Index, y=new_cases/coef), color=deaths_color) +
  labs(x="Data", y="Mortalità", title="Mortalità da COVID-19 in Italia",
       subtitle="Da inizio epidemia") +
  scale_x_date(date_breaks = "1 month", labels=scales::label_date("%d %b'%y")) +
  scale_y_continuous(
    labels = scales::label_percent(),
    sec.axis=sec_axis(~.*coef, name="Nuovi casi/giorno")
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.y.right = element_text(color=deaths_color))
```
