...
set.seed((123))
tibble(
  Categoria=LETTERS[1:10], 
  Conteggio=sort(exp(rnorm(10, 5, 1.5)), decreasing=T), 
  Parziali=cumsum(Conteggio)
) %>%
  ggplot(aes(x=Categoria)) + 
  geom_bar(aes(y=Conteggio, fill=Parziali), stat="identity") + 
  geom_point(aes(y=Parziali)) + 
  geom_line(aes(y=Parziali, group=1)) + 
  geom_label_repel(aes(y=Parziali, label=paste0(round(Parziali/sum(Conteggio)*100), "%")),
                   nudge_x=-0.5, nudge_y=250, size=3) + 
