...
La libreria `stringr` offre numerose funzioni, tra cui le più utili sono quelle per spezzare, per cambiare *case* (maiuscolo/minuscolo) e per efettuare il *patern matching* con espressioni regolari.
...
![https://imgs.xkcd.com/comics/confounding_variables.png](confounding_variables.png){width=3in}
...
Ragionando sul processo fisico, ci rendiamo conto che `Feed` e `Extr` si combinano nella *sezione* di materiale depositato (come `Extr/Feed`), e tanto più essa è piccola, tanto più alta è la probabilità di avere pori. Altresì, le combinazioni $(-1,-1)$ e $(+1,+1)$ dei due fattori producono di fatto la stessa sezione depositata, e quindi rappresentano la stessa condizione di prova.
