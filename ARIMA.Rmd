...
Ora `cpmm` è un oggetto `xts`: la converisone di nuovo verso `ts` può essere fatta così:
...
L'estrazione di sottoinsiemi (subsetting) viene effettuata, anziché con il metodo `window()`, come una semplice indicizzazione (cioè il metodo `[.xts()`). È pussibile usare sia indici numerici (convenzionali) sia stringhe in standard ISO-8601. La data può cioè essere espressa come intervallo:
...
La prima verifica è sempre quella sui dati mancanti. Eliminiamo qualche dato dalla serie `cpm` per vedere, in seguito, come gestre i dati mancanti:
...
Per quanto riguarda i processi $\mathrm{AR}(p)$, essi rappresentano un'autoregressione:
$$
...
```
La serie `dlx` mostra ancora un evodente periodicità stagionale. Questa può essere evidenziata mediante la funzione `monthplot()`, che raggruppa anni diversi per lo stesso mese:
...
Per quanto riguarda i parametri $p$ e $q$, entrambi i diagrammi di autocorrelazione mostrano un forte picco a lag 12 (riprova della stagionalità) e entrambi i grafici mostrano una rapida caduta verso un'oscillazione stabilizzata: dopo un picco a lag 1, sial la PACF che la ACF passano all'oscillazione stabilizzata, quindi $p=1$ e $q=1$.
...
Vediamo ad esempio un processo autoregressivo di tipo $\mathrm{AR}(1)$:
...
Come si vede, la ACF degrada in maniera geometrica entre la PACF ha un brusco calo sotto la soglia di significatività a lag=1, indice appunto di un modello con $p=1$


Simuliao invece un processo a media mobile $\mathrm{MA}(2)$:
...
Se non volessimo utilizzare `auto.arima()`, potremmo verificare l'AIC di una combinazione di parametri esplorati a tappeto. Le funzioni di autocorelazione suggeriscono un modello $\mathrm{ARIMA}(3,1,2)$. Il modello ottimale dovrebbe avere quidi una combinzione di $p$ e $q$ inferiori a 3 e 2. Li proviamo tutti e selezioniamo quello con AIC minore:
