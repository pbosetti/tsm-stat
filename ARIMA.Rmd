---
title: "Serie temporali e ARIMA"
author: "Paolo Bosetti (per TSM)"
output: 
  pdf_document: 
    toc: yes
    fig_caption: yes
...
Processo autoregressivo $\mathrm{AR}(2)$:
...
Processo a media mobile $\mathrm{MA}(2)$:
...
Combinati, $\mathrm{ARMA}(1,2)$:

```{r}
set.seed(123)
x <- arima.sim(model=list(ar=c(0.9), ma=c(0.7, 0.25)), n=1000)
tsdisplay(x)
```
...
```
Le funzioni di autocorelazione suggeriscono un modello $\mathrm{ARIMA}(3,1,2)$. Il modello ottimale dovrebbe avere quidi una combinzione di $p$ e $q$ inferiori a 3 e 2. Li proviamo tutti e selezioniamo quello con AIC minore:

```{r}
g <- expand.grid(p=1:3, q=1:2)
aic <- c()
for (i in 1:dim(g)[1]) {
  aic <- c(aic, arima(diff(s, diff=d), order=c(g$p[i], d, g$q[i])))$aic
}
g[which(aic == min(aic)),]
```
