...
L'ultimo aggiornamento è di seguito generato il giorno `r format(now(), format="%d/%m/%Y")` con dati fino al giorno `r format(index(last(cts)), format="%d/%m/%Y")`. I dati dell'ultima settimana sono i seguenti:

```{r}
last(cts, "1 week") %>% fortify() %>% 
  arrange(desc(Index)) %>%
